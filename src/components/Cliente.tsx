  ...
  return (
    <>
      {abaAtiva === 'Inicio' ? (
        estaFinalizando ? (
          <div className="space-y-6 animate-in slide-in-from-right">
            ...
            <div className="space-y-6 rounded-[40px] border border-zinc-100 bg-white p-10 shadow-sm">
              ...
              <select className="w-full rounded-3xl bg-zinc-50 p-5 font-bold" value={formaPagamento} onChange={(e) => setFormaPagamento(e.target.value)}>
              ...
              <div className="flex justify-between border-t-2 pt-6">
                ...
                <p className="text-3xl font-black text-green-600">R$ {carrinho.reduce((a, i) => a + i.preco, 0).toFixed(2)}</p>
              </div>
                ...
                <div key={loja.id} onClick={() => setLojaSelecionada(loja)} className="group flex items-center rounded-[35px] border border-zinc-100 bg-white p-6 shadow-sm hover:shadow-xl transition-all cursor-pointer">
                  ...
                  <div className="ml-6 flex-1">
                    ...
                    <div className="flex items-center gap-3 mt-3">
                      <span className="text-[9px] font-black text-green-600 bg-green-50 px-2 py-0.5 rounded-md">ABERTO</span>
                      <span className="text-[9px] font-bold text-zinc-400 flex items-center gap-1"><Clock size={10} /> 30-45 MIN</span>
                    </div>
                  </div>
              ...
              {cardapioParaExibir.map(item => (
                <div key={item.id} className="flex items-center justify-between gap-5 rounded-[35px] border border-zinc-100 bg-white p-6 shadow-sm">
                  <div className="flex-1">
                    ...
                    <p className="my-1 text-xs text-zinc-400">{item.descricao}</p>
                    <p className="text-lg font-black text-green-600">R$ {item.preco.toFixed(2)}</p>
                  </div>
            ...
            todosOsPedidos.map(p => (
              <div key={p.id} className="rounded-[40px] border border-zinc-100 bg-white p-8 shadow-sm">
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-xl font-black">{p.lojaNome}</h3>
                ...
                <div className="mt-4 pt-4 border-t flex justify-between font-black">
                   ...
                   <p className="text-[10px] text-zinc-400 uppercase">{p.pagamento}</p>
                </div>
              </div>
            ))
          }
        ...
        <div className="animate-in fade-in duration-500">
          ...
          <div className="mt-8 space-y-1">
            <button className="w-full flex items-center justify-between p-5 bg-white border-b hover:bg-zinc-50 transition-colors group">
              <div className="flex items-center gap-4">
                <div className="p-2 bg-zinc-100 rounded-xl group-hover:bg-orange-100 transition-colors"><MapPin size={20} className="text-zinc-500" /></div>
                <span className="font-bold text-zinc-700">Meus Endereços</span>
              </div>
              <ChevronRight size={18} />
            </button>
            <button onClick={handleLogout} className="w-full flex items-center justify-between p-5 bg-white border-t border-b hover:bg-red-50 mt-4 transition-colors">
              <div className="flex items-center gap-4">
                <div className="p-2 bg-red-50 rounded-xl"><LogOut size={20} className="text-red-500" /></div>
      ...
      {abaAtiva === 'Inicio' && carrinho.length > 0 && !estaFinalizando && (
        <div className="fixed bottom-32 left-6 right-6 z-40 mx-auto max-w-md animate-in slide-in-from-bottom">
